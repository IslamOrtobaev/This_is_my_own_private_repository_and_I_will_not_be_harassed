...
class Cell:
    ...
    def __str__(self):
        return f'Количество ячеек: {self.cell_quantity * "*"}'

    def __add__(self, other):
        return Cell(self.cell_quantity + other.cell_quantity)

    def __sub__(self, other):
        return self.cell_quantity - other.cell_quantity if (self.cell_quantity - other.cell_quantity) > 0 else print('Разность количества ячеек двух клеток меньше нуля!')

    def __mul__(self, other):
        return Cell(int(self.cell_quantity * other.cell_quantity))

    def __truediv__(self, other):
        return Cell(round(self.cell_quantity // other.cell_quantity))


    def make_order(self, cells_in_row):
        row = ''
        for i in range(int(self.cell_quantity / cells_in_row)):
            row += f'{"*" * cells_in_row}\n'
        row += f'{"*" * (self.cell_quantity % cells_in_row)}'
        return row
